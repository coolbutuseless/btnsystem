...
the one command will be run at any call e..g

In the following it can be seen that running the command via processx will 
...
So call using filename expansion with `*` will work correctly in `system2()` 
...
pre-process the arguments to the command:

1. If an arg contains a file expansion character (`*`, `?`, `[`) and no other special
   characters, then leave it as-is.
    * If it also contains spaces, then replace them with escaped spaces i.e. `\ `
...
running multiple commands, while still having special characters which everyting
